...
dl_apkmirror() {
	...
	local base_url="${url%/}"
	url="${base_url}/${apkmname}-${version//./-}-release/"
	if ! resp=$(req "$url" -); then
		local v_trim="${version%.*}"
		if [ "$v_trim" != "$version" ]; then
			url="${base_url}/${apkmname}-${v_trim//./-}-release/"
...
get_github_resp() {
	...
	__GITHUB_RESP__=$(gh_req "https://api.github.com/repos/${repo}/releases" -) || return 1
}
get_github_pkg_name() {
	if [ -n "${args[pkg_name]-}" ]; then
		echo "${args[pkg_name]}"
	elif [ "$__GITHUB_REPO__" = "AdguardTeam/AdguardForAndroid" ]; then
		echo "com.adguard.android"
	else
		return 1
	fi
}
get_github_vers() {
	jq -r '.[].tag_name' <<<"$__GITHUB_RESP__" | sed 's/^v//' | grep -v -i 'beta\|rc\|alpha' || :
}
dl_github() {
	...
	local rel
	rel=$(jq -e -r --arg tag "$tag" --arg ver "$version" '.[] | select(.tag_name == $tag or .tag_name == $ver or .tag_name == ("v" + $ver))' <<<"$__GITHUB_RESP__") || return 1
	local dlurl
	dlurl=$(jq -e -r '.assets[] | select(.name | endswith(".apk") and (contains("TV") | not)) | .browser_download_url' <<<"$rel" | head -1) || return 1
	gh_dl "$output" "$dlurl"
}
		...
		if [ "${args[include_stock]}" != "disable" ]; then
			...
			if [ "${args[include_stock]}" = "merged" ]; then
				...
				if [ -f "${stock_apk}.apkm" ]; then
					...
					pr "Stock apk of $table_name is not a bundle, copying as base.apk"
					cp -f "$stock_apk" "${base_template}/stock/base.apk"
				fi
			fi
		fi
